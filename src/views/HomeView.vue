  ...
  watchEffect(async () => {
    try {
      ...
      const { data } = await axios.get(
        `https://site--leboncoin-backend--2ztmlbwdnwqd.code.run/api/offers?populate[0]=owner.avatar&populate[1]=pictures${priceFilters}&sort=${props.sort}&filters[title][$containsi]=${props.title}&pagination[page]=${props.page}&pagination[pageSize]=10`,
      )
